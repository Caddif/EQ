...
void EQAudioProcessorEditor::timerCallback()
{
    if( parametersChange.compareAndSetBool(false, true))
    {
        ...
        updateCoeffs(monochian.get<Chainpositons::Peak>().coefficients, peakCoeffs);
        ...
        updateCutFilter(monochian.get<Chainpositons::LowCut>(), lowCutCoeffs, chainSettings);
        updateCutFilter(monochian.get<Chainpositons::HiCut>(), hiCutCoeffs, chainSettings);
...
void EQAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    auto& lowCut = monochian.get<Chainpositons::LowCut>();
    auto& peak = monochian.get<Chainpositons::Peak>();
    auto& hiCut = monochian.get<Chainpositons::HiCut>();
    ...
    for (int i = 0; i < graphWidth; ++i)
    {
        ...
        if (!monochian.isBypassed<Chainpositons::Peak>())
